...
class ForagingEnv(gym.Env):
    ...
    def __init__(self, init_pos=False, num_agents=3, obst_pos=np.zeros((1, 3)),
                 map_size=np.array([[0., 0., 0.], [1., 1., 1.]], dtype=np.float64)):
        """
            Description:
    ...
    def reward_collisions(self, dist, labels):
        for i in range(self.num_agents):
            ...
            for j in range(dist[i].shape[0]):
                if dist[i][j] <= 0.15 and dist[i][j] != 0:
                    print('collision ', self.state)
                    print(i, j, dist[i][j])
                    self.reward[i] = -500
                    self.done = True 
    ...
    def reward_target(self):
        for i in range(self.num_agents):
            v = np.linalg.norm(self.target[i] - self.state[i])
            if v < self.min_r:
                self.reward[i] += 1
    ...
    def step(self, action):
        ...
        dist, labels = get_neighbors(self.state, self.num_agents + 1, self.min_r)
        ...
        for i in range(3):
            if (self.state[i] > 1).any() or (self.state[i] < 0).any():   
                print('out of the box ', self.state)
                self.done = True
                self.reward[i] = -500
        ...
        self.reward = [i / 500 for i in self.reward]
        return self.state, self.reward, self.done

    def reset(self, init_pos, obst_pos):
        self.state = np.vstack((init_pos, obst_pos))
        self.done = False
        self.target = init_pos
